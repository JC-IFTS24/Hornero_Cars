document.addEventListener('DOMContentLoaded', function () {
 ...
 function asegurarUsuarioSimuladoEnLocalStorage() {
    ...
    if (!usuarioActivo) {
        console.warn("No hay un usuario en sessionStorage con la clave 'usuarioActivo'.");
        return;
    }

    let usuarios = JSON.parse(localStorage.getItem('usuarios')) || [];

    const index = usuarios.findIndex(u => u.email === usuarioActivo.email || u.userId === usuarioActivo.userId);
}
    ...
    let usuarioCuenta = usuarios.find(u => 
        u.email === usuarioActivo.email || 
        u.userId === usuarioActivo.userId)
    ...
    if (btnCancelarEdicion) { btnCancelarEdicion.addEventListener('click', () => { poblarFormularioMiCuenta(usuarioCuenta); toggleEditMode(false); }); }
    if (formMiCuenta) {
        formMiCuenta.addEventListener('submit', function(event) {
            event.preventDefault(); 
            const datosActualizados = {
                userId: usuarioCuenta.userId, email: usuarioCuenta.email, password: usuarioCuenta.password, fechaRegistro: usuarioCuenta.fechaRegistro,
            ...
            let usuarios = JSON.parse(localStorage.getItem('usuarios')) || [];
            const userIndex = usuarios.findIndex(u => u.userId === usuarioCuenta.userId || u.email === usuarioCuenta.email); 
            if (userIndex !== -1) {
                ...
                usuarioCuenta = {...usuarioCuenta, ...usuarios[userIndex]}; 
                alert('¡Información actualizada con éxito en localStorage!');
            } else {
                usuarioCuenta = datosActualizados; 
                alert('Usuario simulado actualizado en memoria (no se encontró en la lista "usuarios" de localStorage).');
                console.warn("Asegúrate que el 'userId' o 'email' del 'usuarioCuenta' simulado coincida con un usuario registrado para persistencia.");
            }
            toggleEditMode(false); poblarFormularioMiCuenta(usuarioCuenta); 
        });
    }
    ...
    if (btnConfirmarReserva) {
        btnConfirmarReserva.addEventListener('click', function() {
            ...
            if (!usuarioCuenta || !vehiculoParaCotizar || !fechaInicioReservaInput.value || !fechaFinReservaInput.value) {
            ...
            const nuevaReserva = {
                reservaId: reservaId, userId: usuarioCuenta.userId, userNombre: usuarioCuenta.nombreCompleto,
    ...
    function inicializarSeccionInicioDashboard() { if (saludoUsuarioEl && usuarioCuenta) { saludoUsuarioEl.textContent = `¡Hola, ${usuarioCuenta.nombreCompleto.split(' ')[0]}!`; } }
    function inicializarSeccionMiCuenta() { if (usuarioCuenta) { poblarFormularioMiCuenta(usuarioCuenta); } else { console.error("SCRIPT ERROR: No hay datos de currentUser para mostrar en Mi Cuenta."); const miCuentaContent = document.getElementById('mi-cuenta-content'); if (miCuentaContent) { miCuentaContent.innerHTML = "<p class='text-danger'>Error al cargar datos del usuario.</p>"; }} toggleEditMode(false); }
    ...
    function cargarYMostrarMisReservas() {
        ...
        if (!usuarioCuenta || !usuarioCuenta.userId) {
        ...
        const misReservas = todasLasReservas.filter(reserva => reserva.userId === usuarioCuenta.userId).sort((a, b) => new Date(b.fechaCreacion) - new Date(a.fechaCreacion)); 
